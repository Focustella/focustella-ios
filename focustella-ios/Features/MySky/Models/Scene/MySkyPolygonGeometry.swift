...
enum MySkyPolygonGeometry {
    static func hasPolygonOverlap(candidate: Constellation, occupied: [Constellation]) -> Bool {
        let candidateHull = convexHull(points: ConstellationGeometry(constellation: candidate).normalizedPoints)
        return occupied.contains { constellation in
            let occupiedHull = convexHull(points: ConstellationGeometry(constellation: constellation).normalizedPoints)
            return polygonsOverlap(lhs: candidateHull, rhs: occupiedHull)
        }
    }
    ...
    private static func polygonsOverlap(lhs: [CGPoint], rhs: [CGPoint]) -> Bool {
        guard lhs.count >= 3, rhs.count >= 3 else { return false }

        for edgeA in polygonEdges(lhs) {
            for edgeB in polygonEdges(rhs) where segmentsIntersect(edgeA.0, edgeA.1, edgeB.0, edgeB.1) {
                return true
            }
        }
    ...
    private static func polygonEdges(_ polygon: [CGPoint]) -> [(CGPoint, CGPoint)] {
